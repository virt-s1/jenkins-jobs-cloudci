...
[
    ...
    [name: 'RHEL-8.6.0', test_os: 'rhel-8-6', cron: '20 4 * * 4'],
    [name: 'RHEL-9.0.0', test_os: 'rhel-9-0', cron: '20 4 * * 4'],
].each { Map config ->
    pipelineJob("$folderName/${config.name}-gcp-x86_64-image") {
        description("build and upload ${config.name} gcp x86_64 image")
        logRotator(-1, 20, -1, -1)
        parameters {
        ...
        definition {
            cpsScm {
                ...
                lightweight(true)
                scriptPath('cloudci/Jenkinsfile.image.gcp')
            }
        }
    }
}
