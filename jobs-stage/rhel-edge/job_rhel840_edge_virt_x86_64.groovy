...
String folderDescription = 'CI for rhel 8.4.0 edge image building and installation'
// Job name and description
String jobName = 'rhel840_edge_virt_x86_64'
...
pipelineJob("$folderName/$jobName") {
    ...
    parameters {
        ...
        stringParam('TEST_OS', 'rhel-8-4', 'RHEL version')
    }
    properties {
        pipelineTriggers {
            triggers {
                        ...
                        providerDataEnvelope{
                            providerData {
                                activeMQSubscriber {
                                    ...
                                    selector("status = 'FINISHED' AND compose_type = 'nightly' AND release_short = 'RHEL' AND release_version LIKE '8.4%' AND location NOT LIKE '%/updates/%'")
                                }
                            }
                        }
                    }
    ...
    definition {
        cpsScm {
            ...
            lightweight(true)
            scriptPath('Jenkinsfile.rhel840.virt')
        }
    }
}
